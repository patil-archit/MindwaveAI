...
const Layout = ({ children }) => {
    ...
    return (
        <div className="relative min-h-screen overflow-hidden selection:bg-uprock-yellow/30 flex flex-col">
            {/* Navbar (Hidden on Chat Page to keep it clean, or modify style) */}
            {!isChatPage && (
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        ...
                        {currentUser ? (
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={logout} className="text-deep-brown/60 hover:text-deep-brown flex items-center gap-1">
                                    <LogOut size={16} />
                                    Sign Out
                                </button>
                            </div>
                        ...
                        <div className="fixed inset-0 bg-white/95 backdrop-blur-xl z-40 flex flex-col items-center justify-center gap-8 text-2xl font-bold text-deep-brown">
                            ...
                            {currentUser ? (
                                <>
                                    ...
                                    <button onClick={() => { logout(); setIsMenuOpen(false); }}>Sign Out</button>
                                </>
                            ) : (
